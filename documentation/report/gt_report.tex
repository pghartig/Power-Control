...
also performed at individual FC-BSs.
 ...
 $\alpha^{\mathrm{k}}$ is predefined and must satisfy  $\sum_{\mathrm{k=1}}^{\infty} \alpha^{\mathrm{k}}$ (cite).



\end{enumerate} 
Notice that these steps only require the information detailed in the problem description. 

\chapter{Simulation and Results}
\subsection{Simulation System Description}
The Heterogeneous Network is simulated by randomly placing $F $ and M Macro Cell Users within a predefined area. Each FC-BSs is assigned $K_{\mathrm{f}}$ unique Femto Users.
\begin{figure}[h!]
