...
One of the primary goals of this work is to achieve Nash Equilibrium via distributed solutions which minimize system overhead. As seen in the following, the most general setup of this game does not permit such solutions and therefore a refined problem is purposed. Numerical results to the derived solution are then discussed. 
...
\end{itemize}
\item The Potential Function: a centralization of a game into a single optimization problem.
\begin{itemize}
\item
...
\end{equation} In words, $ \Psi()$ is a function whose gradient with respect to the strategy $x$ of any player, is equal to gradient of that players utility function  with respect to their strategy.
\item \textbf{Result:} Global optima to the potential function of a game are Nash Equilibrium.
\item \textbf{Convex Case:} If a Concave N-Person game game admits a potential function, the potential function will be concave allowing for use of convex optimization tools to find a NE. 
\item \textbf{Additional Note:}  $ \Psi() $ is unique (A constant term can be added etc.)
\end{itemize}



\item Distributed Optimization: Allowing for multiple processes to simultaneously solve a central optimzation problem. 
\begin{itemize}
\item \textbf{Steps in Distribution:} Distributed solutions minimize the system overhead required in sharing
\begin{itemize}
\item Begin with a central optimization problem (In this case obtained using a Potential Function to the game).
\item For the dual problem to the central problem
...
\item 
\ref{genproblem}: Analyze the gereral setup to see why further refinements are useful in arriving at a distributed solution. 
\item
...
\item 
	Knowledge of the downlink channel matrix $\mathbf{H_\mathrm{f}} \in \mathbb{C}_{K_f \times T_f} $ to the $K_f$ femto users.
...
If $U_{\mathrm{f,i}}() $ is a general, non-decreasing function, the result is a quasiconvex \cite[p.~102]{BoV:04} function which \emph{maximize}. In the case
...
The general setup of this game seen in the previous section prohibits the use of tools we wish to use in reaching a NE for the game.
...
resulting in the single optimization problem 
	...
	\end{align}
	\end{subequations}
	with $\mathbf{p}= [\mathbf{p_{\mathrm{1}}},\mathbf{p_{\mathrm{2}}}...\mathbf{p_{\mathrm{F}}}]$
...
The lagrangian to the potential function is formed from which the dual problem with be posed. 
\begin{multline}
L(\mathbf{U,\lambda}) = 
\;
\sum_{f=1}^F U_f() 
...
The problem can now be solved using a distributed dual ascent reach the NE of the potential game. 
