...
Vectors are denoted by bold font, lower-case ($\mathbf{x}$) and are assumed column vector.
Matrices are denoted by bold font and capital letters ($\mathbf{X}$).


\section{Relevant Tools/Theory}
It is useful to first review relevant tools for solving such games. Together, these tools provide a series of steps to follow in solving for Nash Equilibrium via distributed solutions. 
...
\item Distributed Optimization: algorithms which solve a central optimization problem using multiple processes in parallel. 
\begin{itemize}
...
\item If the problem is convex with strong duality, the dual ascent will converge to the optimal (In this case a Nash Equilibrium).
